    ...
    <div x-data="{openModalView: false, openModalDelete: false}">
        ...
        <x-modalAlpine modalName="openModalView" title="Show detail data">
            <div class="flex flex-row gap-2">
                <div class="basis-1/4"><img src="{{ $shop['city']['country']['flag'] ?? '' }}" alt="country flag" class="w-full" /></div>
                <div class="basis-1/8">
                    <ul>
                        <li>
                            Shop Name : 🏙️ <span class="font-bold">{{$shop['name'] ?? ''}}</span>
                        </li>
                        <li>
                            City : 🛍️ <span class="font-bold">{{$shop['city']['name'] ?? ''}}</span>
                        </li>
                        <li class="flex content-between gap-2">
                            Country : <span class="font-bold"> {{$shop['city']['country']['name'] ?? ''}}</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <x-modalAlpine modalName="openModalDelete" title="Delete?">
            <div class="p-4 text-center">
                ...
                <button @click="openModalDelete=false" wire:click.prevent="delete({{$shop['id'] ?? null}})" data-modal-hide="popup-modal" type="button" class="btn btn-neutral">
                    Yes, I'm sure
                </button>
