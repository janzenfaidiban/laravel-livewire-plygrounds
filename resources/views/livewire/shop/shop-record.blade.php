    ...
    <div class="card mb-4" x-data="{openModalView: false, openModalDelete: false}">
        ...
        <x-modalAlpine modalName="openModalView" title="Show">
            <div class="px-2 mt-4 mb-4">
                ...
                <div class="mt-3">
                    <p>Name: {{$shop['name'] ?? ''}}</p>
                    <p>City: 	🏙️ {{$shop['city']['name'] ?? ''}}</p>
                    <p>Country: <img src="{{ $shop['city']['country']['flag'] ?? '' }}" alt="flag picture" class="img-fluid" width="34px"> {{$shop['city']['country']['name'] ?? ''}}</p>
                    <p>Created at: {{isset($shop['created_at']) ? \Carbon\Carbon::parse($shop['created_at'])->diffForHumans() : ''}}</p>
                    <p>Updated at: {{isset($shop['created_at']) ? \Carbon\Carbon::parse($shop['created_at'])->diffForHumans() : ''}}</p>
                </div>
            </div>
        </x-modalAlpine>
        ...
        <x-modalAlpine modalName="openModalDelete" title="Are you sure?">
            ...
            <div class="mt-3 flex justify-end gap-2 px-2 mb-4">
                ...
                <button @click="openModalDelete=false" wire:click.prevent="delete({{$shop['id'] ?? null}})" class="bg-white text-red-500 hover:!bg-red-500 hover:text-white border-1 border-red-500 rounded-md py-1 px-2 text-sm">yes, delete!</button>
            </div>
        </x-modalAlpine>
    </div>
</div>
